...
# vincular los Widget de Qt/Pyside con matplotlib. Hay ue agregarla antes de importar
# FigureCanvasQTAgg
...
import numpy as np
import JOULE_BRAYTON.Ciclo_JouleBrayton_V00 as Ciclo_JouleBrayton
import common.PoderCalorifico as PoderCalorifico
import JOULE_BRAYTON.layout_cicloJouleBrayton as layout_cicloJouleBrayton
...
#        Cambio los lineInput de HVS y HVI a "ReadOnly"
    ...
    def plotCiclo(self, puntos):
        ...
        self.axes2.plot(S1, temperatura[1]*np.exp((self.gamma_aire-1)*(S1-entropia[1])
                                                  /(self.R_aire*self.gamma_aire)), 'r')
        self.axes2.plot(S2, temperatura[0]*np.exp((self.gamma_aire-1)*(S2-entropia[0])
                                                  /(self.R_aire*self.gamma_aire)), 'r')
        if self.PostCombustion.checkState() == Qt.CheckState.Checked:
            ...
            self.axes2.plot(S3, temperatura[3]*np.exp((self.gamma_aire-1)*(S3-entropia[3])
                                                      /self.R_aire/self.gamma_aire), 'r')
        self.canvas2.draw()
